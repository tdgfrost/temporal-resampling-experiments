...
if __name__ == "__main__":
    ...
    if plot_patient_example:
        ...
        def draw_panel(ax, ax_ins, data, y_max, show_legend=True, is_bottom_plot=True,
            ...
            y_max_mgdl = y_max
            y_max_mmol = y_max_mgdl / unit_conv
            y_min_mgdl = 10
            y_min_mmol = y_min_mgdl / unit_conv

            # --- A. Gradient Background (remains the same) ---
            y_vals_for_gradient = np.linspace(y_min_mmol, y_max_mmol, 500)
            ...
            def f(x):
                return bg_in_range_magni([x * unit_conv])
            ...
            if show_gradient:
                # Plot Image
                ax.imshow(
                    gradient_image, origin='lower', extent=[x_min, x_max, y_min_mmol, y_max_mmol],
                    aspect='auto', zorder=1
                )

            # --- B. Plot Lines and Targets (remains the same) ---
            LOWER_TARGET_MGDL, HIGHER_TARGET_MGDL = 70, 180
            PEAK_TARGET_MGDL = minimize_scalar(lambda x: -bg_in_range_magni([x]), bounds=(70, 180),
                                          method='bounded').x.item()

            if y_max_mgdl == 700:
                ax.axhline(LOWER_TARGET_MGDL / unit_conv, color='gray', linestyle='--', linewidth=1, zorder=5, label='Target glucose range (3.9–10.0 mmol/L)')
                ax.axhline(HIGHER_TARGET_MGDL / unit_conv, color='gray', linestyle='--', linewidth=1, zorder=5)
                ax.axhline(PEAK_TARGET_MGDL / unit_conv, color='green', linestyle=':', linewidth=1, zorder=5, label='Peak reward')
            ...
            if y_max_mgdl == 250:
                ...
                markerline, stemlines, baseline = ax_cho.stem(
                    pt_time[cho_mask], cho_vals, linefmt='purple', markerfmt='D', basefmt=' ', label='_nolegend_'
                )
                plt.setp(markerline, markersize=5, color='purple', zorder=9)
                plt.setp(stemlines, linewidth=1.5, color='purple', zorder=9)
                ax_cho.set_ylim(y_min_mgdl, y_max_mgdl)
                ax_cho.set_ylabel('Carbohydrates\n(g)', color='purple', fontsize=12)
                ax_cho.tick_params(axis='y', labelcolor='purple', labelsize=10)
                
                # Invisible plot on primary axis to ensure it appears in the legend
                ax.plot([], [], color='purple', marker='D', linestyle='None', label='Carbohydrates')
            ...
            # --- C. Formatting & TICKS ---
            ax.set_ylim(y_min_mmol, y_max_mmol)
        ...
        # Setup Top Insulin Legends exactly where previously placed
        ax_top.get_legend().remove()
        ax_top_ins.get_legend().remove()
        h2, l2 = ax_top_ins.get_legend_handles_labels()
        leg2 = ax_top_ins.legend(h2, l2, loc='upper right', bbox_to_anchor=(0.95, 1.0), fontsize=9, framealpha=0.9)
        ...
        # Setup Bottom Insulin Legends in the exact same location
        h4, l4 = ax_bot_ins.get_legend_handles_labels()
        leg4 = ax_bot_ins.legend(h4, l4, loc='upper right', bbox_to_anchor=(0.95, 1.0), fontsize=9, framealpha=0.9)
        
        # Draw canvas to correctly measure left coordinate of insulin legend
        fig2.canvas.draw()
        try:
            bbox2 = leg2.get_window_extent().transformed(ax_top_ins.transAxes.inverted())
            left_val = bbox2.x0 - 0.01
        except Exception:
            left_val = 0.82  # Fallback

        # Apply left alignment dynamically based on insulin legend left edge
        h1, l1 = ax_top.get_legend_handles_labels()
        ax_top.legend(h1, l1, loc='upper left', bbox_to_anchor=(left_val, 1.0), fontsize=9, framealpha=0.9)
        
        h3, l3 = ax_bot.get_legend_handles_labels()
        ax_bot.legend(h3, l3, loc='upper left', bbox_to_anchor=(left_val, 1.0), fontsize=9, framealpha=0.9)
